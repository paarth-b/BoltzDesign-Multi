...
def boltz_hallucination(
    ...
    # Handle multiple YAML files for multi-state design
    # If yaml_path is a list, we load all of them. If it's a single path, we load just one.
    yaml_paths = yaml_path if isinstance(yaml_path, list) else [yaml_path]
    multi_state_data = []
    
    for yp in yaml_paths:
        with yp.open("r") as file:
            data = yaml.safe_load(file)
        # Initialize binder sequence to X for all states
        data['sequences'][chain_to_number[binder_chain]]['protein']['sequence'] = 'X'*length
        multi_state_data.append(data)

    # Use the name of the first file for schema parsing / identification
    name = yaml_paths[0].stem
    ...
    # Masks are specific to each batch (structure), but binder mask is consistent
    # We use the first batch for defining the optimization mask
    mask = torch.ones_like(batches[0]['res_type_logits'])
    mask[batches[0]['entity_id']!=chain_to_number[binder_chain], :] = 0
    
    # We need chain masks for all batches for loss calculation
    chain_masks = [(b['entity_id'] == chain_to_number[binder_chain]).int() for b in batches]
    ...
    def design(batches, 
        ...
        def update_sequence(opt, batches, mask, alpha=2.0, non_protein_target=False, binder_chain='A'):
            ...
            # batches[0]['res_type_logits'] is the optimizer parameter.
            # We perform the softmax logic on this shared tensor, then broadcast the result (res_type) to all batches.
            
            # Use logits from the first batch (they are identical objects/pointers across batches)
        ...
        for i in range(iters):
            ...
            batches = update_sequence(opt, batches, mask, non_protein_target=non_protein_target, binder_chain=binder_chain)
            ...
            current_sequence = ''.join([alphabet[i] for i in torch.argmax(batches[0]['res_type'][batches[0]['entity_id']==chain_to_number[binder_chain],:], dim=-1).detach().cpu().numpy()])
    ...
    # For pre-run, just return metrics based on the first state/batch for simplicity
    if pre_run:
        best_logits = batches[0]['res_type_logits']
        best_seq = ''.join([alphabet[i] for i in torch.argmax(batches[0]['res_type'][batches[0]['entity_id']==chain_to_number[binder_chain],:], dim=-1).detach().cpu().numpy()])
        # Update sequence in all data objects
        for d in multi_state_data:
             d['sequences'][chain_to_number[binder_chain]]['protein']['sequence'] = best_seq
             
        return batches[0]['res_type'].detach().cpu().numpy(), plots, loss_history, distogram_history, sequence_history, traj_coords_list, traj_plddt_list

    boltz_model.eval()

    # For final validation, we can just return the first batch as "best_batch" structure for downstream compatibility
    # or return a list of all best batches if the pipeline supports it.
    # The prompt requests minimal changes to output structure, so we adhere to returning single items where appropriate
    # but the sequence is consensus.
    ...
    # Use first batch logits for final mutation (they are shared anyway)
    best_logits = batches[0]['res_type_logits']
    best_seq = ''.join([alphabet[i] for i in torch.argmax(batches[0]['res_type'][batches[0]['entity_id']==chain_to_number[binder_chain],:], dim=-1).detach().cpu().numpy()])
    ...
    # Create apo data for all states
    multi_state_data_apo = []
    for d in multi_state_data:
        d_apo = copy.deepcopy(d)
        d_apo.pop('constraints', None)
        d_apo['sequences'] = [d_apo['sequences'][chain_to_number[binder_chain]]]
        multi_state_data_apo.append(d_apo)

    def _update_batches_all(data_list, data_apo_list):
        new_batches = []
        new_structures = []
        new_batches_apo = []
        new_structures_apo = []
        
        for d, d_apo in zip(data_list, data_apo_list):
             t = parse_boltz_schema(name, d, ccd_lib)
             t_apo = parse_boltz_schema(name, d_apo, ccd_lib)
             
             b, s = get_batch(t, msa_max_seqs, length, keep_record=True)
             b_apo, s_apo = get_batch(t_apo, msa_max_seqs, length, keep_record=True)
             
             b = {key: value.unsqueeze(0).to(device) if key != 'record' else value for key, value in b.items()}
             b_apo = {key: value.unsqueeze(0).to(device) if key != 'record' else value for key, value in b_apo.items()}
             
             new_batches.append(b)
             new_structures.append(s)
             new_batches_apo.append(b_apo)
             new_structures_apo.append(s_apo)
    ...
    return output, output_apo, best_batch, best_batch_apo, best_structure, best_structure_apo, distogram_history, sequence_history, loss_history, con_loss_history, i_con_loss_history, plddt_loss_history, traj_coords_list, traj_plddt_list, best_structure
...
def run_boltz_design(
    ...
    # Logic to grouping yaml files by base name (e.g. 5zmc_0.yaml, 5zmc_1.yaml -> 5zmc)
    yaml_groups = {}
    for yaml_path in Path(yaml_dir).glob('*.yaml'):
        stem = yaml_path.stem
        # Assumption: multi-state files end with _0, _1 etc.
        if '_' in stem and stem.split('_')[-1].isdigit():
             base_name = "_".join(stem.split('_')[:-1])
        else:
             base_name = stem
        
        if base_name not in yaml_groups:
