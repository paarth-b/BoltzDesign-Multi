...
class OuterProductMean(nn.Module):
    ...
    def forward(self, m: Tensor, mask: Tensor, chunk_size: int = None) -> Tensor:
        ...
        """
        # Expand mask
        mask = mask.unsqueeze(-1).to(device=m.device, dtype=m.dtype)
        ...
        if chunk_size is not None and not self.training:
            ...
            for i in range(0, self.c_hidden, chunk_size):
                ...
                # Project to output
                if i == 0:
                    z_out = z.to(device=m.device, dtype=m.dtype) @ sliced_weight_proj_o.T
                else:
                    z_out = z_out + z.to(device=m.device, dtype=m.dtype) @ sliced_weight_proj_o.T
            ...
            # Project to output
            z = self.proj_o(z.to(device=m.device, dtype=m.dtype))
            return z
